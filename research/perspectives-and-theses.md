...
## 11. 2026-09-21：从动作后果到可恢复性（待验证）

XPACE与EmbodiedJev提示一个更具体的问题：动作偏离后，表示能否识别参考策略仍可挽回的状态，并帮助选择值得学习的恢复经验？这里的可恢复性依赖固定参考策略、执行接口和预算；它不是普适物理常数，也不因命名就不同于value。

先用独立物理执行给标签，区分失败监督与成功动作模仿；再做表示×数据选择的匹配实验。若仅恢复数据有效，应承认是数据收益；若表示在相同数据下还有增益，才继续检验物理变化和新任务迁移。详见[候选实验方案](xpace-jev-rlt-ideas.md)。
